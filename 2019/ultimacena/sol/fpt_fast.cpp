// Soluzione "FPT fast" per vertex cover.
// Come "FPT":
...
// L'approccio di questa versione e' detto «kernelized» perche' riduce il problema
// della copertura al "kernel" del grafo. Infatti ogni vertice di grado >= 11 deve
// essere sicuramente incluso nella copertura.
...
int riassumi(int N, int M, int A[], int B[], int S[]) {
    ...
    // Preprocess: any vertex with degree >= 11 must be included in the cover
    for (int i = 0; i < N; i++)
        if (graph[i].size() >= 11)
            cover.insert(i);
    for (int i = 0; i < M; i++)
        if (cover.find(edges[i].first) != cover.end() or
                cover.find(edges[i].second) != cover.end())
            skip_edge[i] = true;
